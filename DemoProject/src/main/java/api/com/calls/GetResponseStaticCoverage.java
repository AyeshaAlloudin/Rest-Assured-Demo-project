...
public class GetResponseStaticCoverage {
	Properties pro = new Properties();

	// This will fetch the response body as is and log it. given and when are
	// optional here
	public void getstaticcoverage() {
		Response res = RestAssured.given().header("x-api-key", pro.ApiKey).when().get(pro.getStaticCoverage).then()
				.extract().response();
